...
int main(int argc, char *argv[])
{
    ...
    if (argc != 7)
    {
        fprintf(stderr, "Usage:  %s <Multicast addr> <Multicast port> <Client port> <Number of sectors> <Sector with trasure> <Number of clients>\n", argv[0]);
        exit(1);
    }
    multicastIP = argv[1];
    multicastPort = atoi(argv[2]);
    port = atoi(argv[3]);
    int sectorsCount = atoi(argv[4]);
    int trasureSector = atoi(argv[5]);
    clients_count = atoi(argv[6]);
    trasureSector -= 1;
    ...
    while(!printed) {
        sendto(multicast_sock, sectors, sizeof(int) * sectorsCount, 0, (struct sockaddr *) &multicastAddr, sizeof(multicastAddr));
        sleep(1);
    }
    sectors[0] = -1;
    sendto(multicast_sock, sectors, sizeof(int) * sectorsCount, 0, (struct sockaddr *) &multicastAddr, sizeof(multicastAddr));
